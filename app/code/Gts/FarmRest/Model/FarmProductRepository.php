 ...
 */
class FarmProductRepository implements \Gts\FarmRest\Api\FarmProductRepositoryInterface
{
    /**
    ...
    public function getFarmList()
    {
        ...
        $newResult = [];
        $farm_type = array("219"=>"葉菜","220"=>"瓜果","221"=>"乾貨");
        $productArray = $collection->toArray();
        foreach ($productArray as $product) {
            $tmp = [];
            $tmp ['id'] = $product['entity_id'];
            $tmp ['name'] = $product['name'];
            $tmp ['sku'] = $product['sku'];
            $tmp ['price'] = $product['price'];
            $tmp ['weight'] = $product['weight'];
            $tmp ['farm_type'] = $farm_type[$product['farm_type']];
            array_push($newResult, $tmp);
        }
        return $newResult;
    }

    public function getFarmListById($productId)
    {
        $farm_type = array("219"=>"葉菜","220"=>"瓜果","221"=>"乾貨");
        $product = $this->getById($productId);
        $newResult = [];
        $newResult['id'] = $productId;
        $newResult['price'] = $product->getPrice();
        $newResult['weight'] = $product->getWeight();
        $newResult['sku'] = $product->getSku();
        $newResult['name'] = $product->getName();
        $newResult['farm_type'] = $farm_type[$product->getCustomAttribute('farm_type')->getValue()];
        return [$newResult];
    }
